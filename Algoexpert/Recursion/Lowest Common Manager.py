...
def getLowestCommonManager(topManager, reportOne, reportTwo):
    ...
    stackOne = getManagers(topManager, reportOne, [topManager])
    stackTwo = getManagers(topManager, reportTwo, [topManager])
    ...
    if report.directReports:
        for sub in report.directReports:
            ...
            if stack is not None:
                print(stack[-1], ' is the targetReport. Current stack frame on node: ', report.name)
                return stack
            elif stack is None:
                testNode = tempstack.pop()
                print(testNode, ' is not the targetReport')
                stack = tempstack
...
C.directReports = [F, G]
ans = [A]
print('A and I\'s manager:', getLowestCommonManager(A, I, A))
ans = [A.name]
